...
var minOperations = function (nums, x) {
    // 在没有减成负数的情况下，哪边大减哪边：这个方法不正确
    const helper = (nums, x) => {
        let ans = 1;
        if (x === 0) {
            return ans;
        }
        const lNum = nums.shift();
        let lRes = x < lNum ? Number.POSITIVE_INFINITY : helper(nums, x - lNum);
        nums.unshift(lNum);
    
        const rNum = nums.pop();
        let rRes = x < rNum ? Number.POSITIVE_INFINITY : helper(nums, x - rNum);
        nums.push(rNum);
        return ans + Math.min(lRes, rRes);
    };
    
    const res = helper(nums, x);
    console.log(res);
};

// console.log(Math.min(Number.POSITIVE_INFINITY, Number.POSITIVE_INFINITY));
